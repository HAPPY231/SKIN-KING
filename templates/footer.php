<footer>
<div class="box">
    <div class="boxo">
        <div class="in-box1">
            <img class="logo" src="images/Skin-King1.jpg" width="100%" style="min-width:120px;"
                 alt="logo">
        </div>
        <div class="in-box2">
            <div class="s">
                <span class="pan">Skiny</span>
                <a href="Equipment"> <span class="pod">Ekwipunek</span></a>
                <a href="Settings"> <span class="pod">Ustawienia konta</span></a>
                <a href="Lottery"> <span class="pod">Doładowanie</span></a>
            </div>
            <div class="s">
                <span class="pan">Giveaway</span>
                <a href="#"> <span class="pod">Regulamin</span></a>
                <a href="#"> <span class="pod">Zasady</span></a>
                <a href="#"> <span class="pod">Nagrody</span></a>
            </div>
            <div class="s"><span class="pan">Pomoc</span>
                <a href="#"> <span class="pod">Kontakt</span></a>
                <a href="#"> <span class="pod">Regulamin</span></a>
                <a href="#"> <span class="pod">Uwagi</span></a>
            </div>
        </div>
    </div>
</div>
</footer>
</body>
</html>
